  ...
  properties: {
    products: {
      ...
      items: {
        ...
        properties: {
          ...
          nutrition: {
            ...
            properties: {
              ...
              carbohydrate_g: { type: "string" },
              sugars_g: { type: "string" },
              added_sugars_g: { type: "string" },
              fat_g: { type: "string" },
              saturated_fat_g: { type: "string" },
              fibre_g: { type: "string" },
        ...
        required: [
          "name","brand","sku","pack_size","category","ingredients",
          "nutrition","source_urls","source_notes","verification_status"
        ]
      }
    },
...
function citationsFrom(data) {
  const out = [];
  for (const step of arr(data?.steps)) {
    for (const block of arr(step?.content)) {
      for (const a of arr(block?.annotations)) {
        const url = clean(a?.url || a?.uri);
        if (url) out.push({ url, title: clean(a?.title) });
      }
    }
  }
  const seen = new Set();
...
async function callGeminiResearch(env, prompt) {
  ...
  if (!key) throw new Error("SECRET_MISSING: GEMINI_API_KEY is not reaching the active Worker runtime.");
  ...
  if (!response.ok) {
    if (response.status === 429) {
      throw new Error("GEMINI_RATE_LIMITED: Gemini rate/quota limit was reached. Please try again later.");
    }
    throw new Error(`GEMINI_API_ERROR: HTTP ${response.status}. ${clean(data?.error?.message) || raw.slice(0, 500)}`);
  }
...
function normalize(p, citations) {
  const n = p?.nutrition || {};
  const own = arr(p?.source_urls).map(clean).filter(Boolean);
  const urls = [...new Set([...own, ...citations.map(x => x.url)])].slice(0, 8);
  return {
    name: clean(p?.name),
    brand: clean(p?.brand),
    sku: clean(p?.sku),
    ...
    ingredients: arr(p?.ingredients).map(clean).filter(Boolean),
    nutrition: {
      serving_size: clean(n.serving_size),
      energy_kcal: clean(n.energy_kcal),
      protein_g: clean(n.protein_g),
      carbohydrate_g: clean(n.carbohydrate_g),
      sugars_g: clean(n.sugars_g),
      added_sugars_g: clean(n.added_sugars_g),
      fat_g: clean(n.fat_g),
      saturated_fat_g: clean(n.saturated_fat_g),
      fibre_g: clean(n.fibre_g),
      sodium_mg: clean(n.sodium_mg)
    },
    source_urls: urls,
    source_notes: arr(p?.source_notes).map(clean).filter(Boolean),
    verification_status: clean(p?.verification_status) || "partially_verified",
    kidposhan_score: null
  };
}

export async function researchProducts(env, input = {}) {
  const result = await callGeminiResearch(env, promptFor(input));
  let parsed;
  try {
    parsed = parseJson(result.text);
  } catch (e) {
    throw new Error(`GEMINI_RESPONSE_PARSE_ERROR: ${e.message}`);
  }

  const products = arr(parsed?.products)
    .map(p => normalize(p, result.citations))
    .filter(p => p.name);
  ...
  return {
    ...
    research_notes: arr(parsed?.research_notes).map(clean).filter(Boolean),
    products,
    citations: result.citations,
    score_status: "not_scored",
    score_message: "Verified product facts are returned without inventing a KidPoshan Score. The existing deterministic score engine will calculate the score in the next layer."
  };
}
...
export function geminiRuntimeStatus(env) {
  return {
    ...
    secret_present: typeof env?.GEMINI_API_KEY === "string" && env.GEMINI_API_KEY.trim().length > 0
  };
}
